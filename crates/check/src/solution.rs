    ...
    SA::Error: Send,
{
    ...
    let new_node = |node_ix: usize| Node {
        ...
        reads: predicate.nodes[node_ix].reads,
    };
    ...
    SB: StateReadSync<Error = SA::Error>,
{
    ...
    let gas_spent = match reads {
        Reads::Pre => vm.exec_ops_sync(&ops, access, &pre_state, &gas_cost, gas_limit)?,
        Reads::Post => vm.exec_ops_sync(&ops, access, &post_state, &gas_cost, gas_limit)?,
    };
